...
public class NutritionDay
{
    ...
    {
        AddFoodToMeal(food);
        AddNutrients(food, NutrientTotals);
    }

    public void RemoveFood(FoodItem food)
    {
        switch (food.MealOfDay)
        {
            case MealOfDay.NoClassification:
                break;

            case MealOfDay.Breakfast:
                BreakfastFoods.Remove(food);
                break;

            case MealOfDay.Lunch:
                LunchFoods.Remove(food);
                break;

            case MealOfDay.Dinner:
                DinnerFoods.Remove(food);
                break;

            case MealOfDay.Snacks:
                SnacksFoods.Remove(food);
                break;
        ...
        SubtractNutrients(food, NutrientTotals);
    }
    ...
    private static void AddNutrients(FoodItem food, Dictionary<string, double> nutrients)
    {
    ...
    private static void SubtractNutrients(FoodItem food, Dictionary<string, double> nutrients)
    {
        var amount = food.Amount;
        nutrients["Calories"] -= CalculateNutrients(food.Calories, amount);
        nutrients["Protein"] -= CalculateNutrients(food.Protein, amount);
